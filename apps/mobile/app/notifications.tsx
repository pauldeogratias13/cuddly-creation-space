  ...
  const handleNotificationPress = (notification: any) => {
    ...
    switch (notification.type) {
      ...
      case 'follow_request':
        ...
        break;
      case 'chat_message':
        if (notification.data?.chat_thread_id) {
          router.push(`/chat/${notification.data.chat_thread_id}`);
        }
        break;
      case 'video_like':
      case 'video_comment':
        if (notification.data?.video_id) {
          router.push(`/video/${notification.data.video_id}`);
        }
        break;
      case 'profile_view':
        if (notification.actor_id) {
      ...
      <View className="flex-row items-start">
        ...
        <View className="flex-1">
          ...
          {item.actor && (
            <View className="flex-row items-center">
              <Image
                source={{ uri: item.actor.avatar_url || 'https://api.dicebear.com/7.x/identicon/svg?seed=' + item.actor.username }}
