...
type DiscoverContent = {
  id: string;
  type: "video" | "photo" | "article" | "music";
  title: string;
  thumbnail: string;
  author: string;
  likes: number;
...
export default function DiscoverScreen() {
  ...
  const discoverContent: DiscoverContent[] = [
    {
      id: "1",
      type: "video",
  ...
  const renderCategory = ({ item }: { item: Category }) => (
    ...
    >
      <Ionicons name={item.icon} size={24} color={activeCategory === item.id ? "white" : "#666"} />
      <Text
        className={`text-xs mt-1 ${activeCategory === item.id ? "text-white" : "text-gray-600"}`}
      >
        {item.label}
      </Text>
    </TouchableOpacity>
  ...
  const renderUser = ({ item }: { item: SuggestedUser }) => (
    <View
      className="bg-white border border-gray-200 rounded-lg p-4 mr-3 mb-3"
      style={{ width: 200 }}
    >
      ...
      <TouchableOpacity
        className={`py-2 rounded-lg ${
          item.is_following ? "bg-gray-200 border border-gray-300" : "bg-blue-500"
        }`}
      >
        <Text
  ...
  const renderContent = ({ item }: { item: DiscoverContent }) => (
    <TouchableOpacity
      className="bg-white rounded-lg overflow-hidden mr-3 mb-3"
      style={{ width: 200 }}
    >
      <View className="relative">
        <Image source={{ uri: item.thumbnail }} className="w-full h-32" resizeMode="cover" />
        {item.type === "video" && (
