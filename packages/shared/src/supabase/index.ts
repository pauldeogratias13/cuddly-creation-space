...
export const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.EXPO_PUBLIC_SUPABASE_URL || '';
...
export const uploadVideo = async (videoData: Database['public']['Tables']['public_videos']['Insert']) => {
  const { data, error } = await supabase
    .from('public_videos')
    .insert(videoData)
    .select()
...
export const addToCart = async (itemData: Database['public']['Tables']['commerce_cart']['Insert']) => {
  const { data, error } = await supabase
    .from('commerce_cart')
    .insert(itemData)
    .select()
...
export const updateCartItem = async (itemId: string, updates: Database['public']['Tables']['commerce_cart']['Update']) => {
  const { data, error } = await supabase
    .from('commerce_cart')
    .update(updates)
    .eq('id', itemId)
...
export const createOrder = async (orderData: Database['public']['Tables']['commerce_orders']['Insert']) => {
  const { data, error } = await supabase
...
export const subscribeToChatMessages = (threadId: string, callback: (payload: any) => void) => {
  return supabase
